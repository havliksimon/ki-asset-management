...
def generate_portfolio_chart_series(analysis_ids: List[int], years: Optional[int] = None) -> Dict:
    """
    Generate portfolio performance chart data with monthly datapoints.
    Similar to main page chart with proper time series.
    ...
    # Sort by analysis date and get earliest
    analyses = sorted(analyses, key=lambda x: x.analysis_date)
    earliest_date = analyses[0].analysis_date
    
    # Limit to requested years if specified
    if years:
        cutoff = date.today() - timedelta(days=years*365)
        if cutoff > earliest_date:
            earliest_date = cutoff
            analyses = [a for a in analyses if a.analysis_date >= cutoff]
