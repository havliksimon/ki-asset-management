...
def get_portfolio_chart_data():
    ...
    """
    from ..models import Vote, StockPrice, Company
    try:
        import pandas as pd
        ...
        # Create monthly date points from inception to today
        end_date = date.today()
        ...
        for d in dates:
            ...
            # Get benchmark returns for this date
            spy_ret = _get_benchmark_return_for_date('SPY', earliest_date, current_date)
            vt_ret = _get_benchmark_return_for_date('VT', earliest_date, current_date)
            eems_ret = _get_benchmark_return_for_date('EEMS', earliest_date, current_date)
            
            spy_series.append(spy_ret)
            vt_series.append(vt_ret)
            eems_series.append(eems_ret)
